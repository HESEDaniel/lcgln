...
class GlobalICNN(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(GlobalICNN, self).__init__()
    ...
    def __init__(
        ...
        for _ in range(num_hidden_layers):
            self.hid_layers.append(
                PositiveLinear(hidden_dim, hidden_dim),      # TODO: nonneg const!!!
            )
            self.hid_layers_for_input.append(
                nn.Linear(input_dim, hidden_dim, bias=False),
            )
            
        self.output_layer = PositiveLinear(hidden_dim, output_dim)            # TODO: nonneg const!!!, bias=False
...
class EGLWeightedMSE(torch.nn.Module):
    ...
    """
    def __init__(self, X, Y, min_val=1e-3, **kwargs):
...
class EGLDirectedQuadratic(torch.nn.Module):
    ...
    def forward(self, data_idx, Yhats):
        ...
        quad = (diff @ self.matrix).square().sum(dim=-1).squeeze()
...
class ICNNPlusPlus(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(ICNNPlusPlus, self).__init__()
        self.Y_raw = Y.detach()
        self.Y = torch.nn.Parameter(self.Y_raw.view((-1)))
        self.num_dims = self.Y.shape[0]
