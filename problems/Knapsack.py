...
class Knapsack(PThenO):
    ...
    def __init__(
        ...
        num_items=2,  # number of targets to consider
        ...
        # self.Xs_train = torch.rand(self.num_train_instances, self.num_items, 2)
        
        self._set_seed(test_seed)
        self.Xs_test = 2 * torch.rand(self.num_test_instances, self.num_items, 1) - 1
        # self.Xs_test = torch.rand(self.num_test_instances, self.num_items, 2)

        #   Generate Labels
        self.Ys_train = 10 * (self.Xs_train.pow(3) - 0.75 * self.Xs_train).squeeze()
        self.Ys_test = 10 * (self.Xs_test.pow(3) - 0.75 * self.Xs_test).squeeze()
        # self.Ys_train = 10 * (self.Xs_train[:,:,0] - 3*self.Xs_train[:,:,0].pow(2)*self.Xs_train[:,:,1] + self.Xs_train[:,:,1]).pow(2).squeeze()
        # self.Ys_test = 10 * (self.Xs_test[:,:,0] - 3*self.Xs_test[:,:,0].pow(2)*self.Xs_test[:,:,1] + self.Xs_test[:,:,1]).pow(2).squeeze()
