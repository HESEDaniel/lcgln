...
class OptimiseSubmodular(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        # Apply chain rule
        dZdYhat_t = dZdYhat.t()
        out = torch.mm(dZdYhat_t.float(), grad_output.cuda().view(len(Z), 1))
    ...
    def _get_dZdYhat(
        ...
        b = torch.vstack([torch.hstack([dfdZ_dYhat.view((num_var, Yhat.numel()))]), torch.hstack([torch.zeros((num_constr, Yhat.numel()))]).cuda()])

        # solution to the system
        derivatives = torch.linalg.solve(A.cuda(), b)
